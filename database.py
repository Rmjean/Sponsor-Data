"""
This program an example of database and entry form
...
def help():
    help_screen = tkinter.Toplevel(screen)
    screen.title('Help ?')
    screen.configure(bg="white")
    photo = tkinter.PhotoImage(file = "assets\login_icon.png")
    screen.iconphoto(False, photo)
    
    help_txt = tkinter.Label(help_screen, text="""
1.) Enter data into the bar on the left
    - Email must contain an @ and .com 
    - Phone number must contain exactly 10 numbers

2.) To seach the database click dropdown box above seach bar to select column to seach by

3.) When editing an item all information is removed from the table and inserted into the entry bar onto the left. 
(If you leave without enter data the data will be lost)

4.) Remove Sponsor will delete any selected item from the table

5.) Click Exit to leave the application
...
database_tree.column('Name',width=125)
database_tree.column('Type', width=175)
database_tree.column('Resources', width=125)
database_tree.column('Contact', width=125)
database_tree.column('Email', width=125)
database_tree.column('Phone', width=125)
